	...
	class Puc_v4p10_Plugin_Ui {
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->updateChecker->slug) ) {
				...
				if ( $status == 'no_update' ) {
					$message = 'آپدیت جدیدی برای نسخه موبایل نگار در دسترس نیست';
				} else if ( $status == 'update_available' ) {
					$message = 'آپدیت جدیدی برای نسخه موبایل نگار منشتر شده. ابتدا قالب را با استفاده از بروزرسان خودکار سایت راستچین آپدیت کنید و سپس در منوی افزونه ها ، پلاگین نگار را بروزرسانی کنید';
				} else if ( $status === 'error' ) {
					$message = 'خطایی رخ داد. کمی بعد مجدد تست کنید';
